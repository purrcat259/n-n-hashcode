...
class Process:
    ...
    def overlap_exists(self, slice):
        ...
        br_sl_col = slice.top_left[1] + slice.cols - 1
    ...
    def is_point_in_area(self, coor, area_tl, area_br):
        return area_tl[0] <= coor[0] and coor[0] <= area_br[0] and
                area_tl[1] <= coor[1] and coor[1] <= area_br[1]

    def is_below_minimum_ingredients(self, slice):
        return slice.mushroom >= self.min_ing && slice.tomato >= self.min_ing
